...
PROPOSALS <- rbind(
  ...
  data.frame(object = "PARAM_CATALOGUE", key = "Milk", field = "ipcc_default",
    proposed = "(confirm)", status = "PROPOSED",
    why = "Correct against Table 10A.1 Africa dairy. Flagged only because it sits in the dairy/non-dairy mixing cluster: the catalogue draws Milk, Fat and Ym from the dairy row and BW and CP from the non-dairy grazing row, describing no animal IPCC published. Adjudicated at review round 8 page 7, so unchanged."),
  data.frame(object = "PARAM_CATALOGUE", key = "Fat", field = "ipcc_default",
    proposed = "(confirm)", status = "PROPOSED",
    why = "As Milk: correct against Table 10A.1 Africa dairy, but part of the same mixing question. Adjudicated at review round 8 page 7, so unchanged."),
