...
assemble_translator_system_prompt <- function(asset_dir = "claude_project_assets") {
  ...
  marker_instruction <- paste(
    ...
    "",
    "Even when ready, prefer telling the user to click 'Produce template",
    "now' over inlining the block in streaming chat — see in-app",
    "presentation rule 9. Inlining a multi-sub-category template here",
    "hits the streaming token cap and the parser rejects the truncated",
    "block. The button path is the reliable one.",
    sep = "\n")
  ...
  ui_presentation_rules <- paste(
    ...
    "",
    "9. **NEVER inline the `template-ready` JSON block in the streaming",
    "   chat reply.** A complete filled inventory is 200+ JSON rows; it",
    "   does not fit in the streaming chat's token budget and gets",
