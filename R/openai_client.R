...
assemble_translator_system_prompt <- function(asset_dir = "claude_project_assets") {
  ...
  ui_presentation_rules <- paste(
    ...
    "   response to do the work and then stop talking. Either:",
    "     (a) emit the `template-ready` JSON block in THIS reply, right",
    "         now, in full, as the final part of your message; or",
    "     (b) ask the user to click the green 'Produce template now'",
    "         button below the chat — say so in one short sentence and",
    "         end the reply.",
    "   Do NOT promise to generate in 'the next message'. There is no",
    "   next message unless the user types one. A stall ends the turn",
    "   with the user staring at silence.",
    "",
