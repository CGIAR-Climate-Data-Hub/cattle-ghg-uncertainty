...
<!-- Values in double braces ({{...}}) are filled in by the app from its live defaults when this prompt is assembled, so this file never carries a number that can go stale. See R/translator_prompt_build.R. -->
...
- `param_catalogue` — the {{n_params}} IPCC-aligned parameters (codes, units, defaults, distributions, accepted aliases) and the assumptions those defaults make. Generated from the app's own tables.
...
Every catalogue parameter NOT present in the file: the set difference of {{{params}}} minus what is in section B. These take IPCC defaults at emission.
...
- Sub-category vocabulary mapping (raw label → controlled vocabulary: {{subcats}})
...
1. **Every catalogue parameter, for every sub-category in the inventory**, but ONLY where the file does not already supply a value. The catalogue parameters: {{params}}.
2. **`pct_pregnant` when the file does not give it**: `dairy_cows` → {{default:pct_pregnant:dairy_cows}}; `other_cows` → {{default:pct_pregnant:other_cows}}; `heifers` (if pregnant heifers are bundled here) → {{default:pct_pregnant:heifers}} (a project assumption: Table 10A.2 leaves the cell blank); males, calves and feedlot cattle → 0 with `data_source = "biological_zero"`.
...
4. **Set `species` from the sub-categories you actually mapped — never `cattle_mixed` as a hedge.** `dairy_cows` AND any of {{subcats_non_dairy}} → `cattle_mixed`; `dairy_cows` only → `cattle_dairy`; no `dairy_cows` → `cattle_non_dairy` (the common case for beef-only and smallholder inventories).
...
9. **Sex-specific coefficients** from the override table in `param_catalogue` are applied: `bulls.C` = {{default:C:bulls}} (not the female {{default:C:dairy_cows}}), `oxen.C` = {{default:C:oxen}}, `growing_males.C` = {{default:C:growing_males}}; `bulls.Cfi` = {{default:Cfi:bulls}}; `oxen.Cfi` and `growing_males.Cfi` = {{default:Cfi:oxen}} (non-lactating), not the lactating {{default:Cfi:dairy_cows}}. Tag these `ipcc_default` and mention the override in your summary.
10. **HARD ROW-COUNT ASSERTION (last, immediately before calling the tool).** `parameters.length` must equal (number of confirmed sub-categories in this piece) × {{n_params}}. If it doesn't, you skipped rows: walk back through section B, add the missing (sub_category, parameter) pairs with the right `data_source`, and only then call the tool. There is no "for brevity" exception. The output budget is large enough for 650+ rows.
...
{{params}}. Always consult `param_catalogue` for definitions, units and defaults; do not paraphrase from memory.
